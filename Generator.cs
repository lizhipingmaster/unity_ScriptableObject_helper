...
namespace CSObjectWrapEditor
{
    ...
    public struct XLuaTemplates
    {
        ...
        public XLuaTemplate GenClassEditor;
    }
    ...
    public static class Generator
    {
        ...
        public static XLuaTemplates templateRef;

        static Generator()
        {
#if !XLUA_GENERAL
            TemplateRef template_ref = ScriptableObject.CreateInstance<TemplateRef>();
            //var ta = AssetDatabase.LoadAssetAtPath<TextAsset>("Assets/XLua/Src/Editor/Template/GenClassEditor.tpl.txt");
            template_ref.GenClassEditor = AssetDatabase.LoadAssetAtPath<TextAsset>(/*Application.dataPath + */"Assets/XLua/Src/Editor/Template/GenClassEditor.tpl.txt");
            ...
            templateRef = new XLuaTemplates()
            {
                ...
                GenClassEditor = { name = template_ref.GenClassEditor.name, text = template_ref.GenClassEditor.text },
            };
#endif
        ...
        static public void GenOne(Type type, Action<Type, LuaTable> type_info_getter, XLuaTemplate templateAsset, StreamWriter textWriter)
        {
...
#pragma warning restore 618
        ...
        [MenuItem("XLua/LuaClass", false, 2)]
        public static void LuaClass()
        {
            luaenv.DoString("require 'TemplateCommon'");
            var lines = File.ReadAllLines(Application.dataPath + "/Scripts/UI/process.txt");
            Debug.Log(lines[lines.Length - 1]);

            List<string> files = new List<string>(lines);
            var all = XLua.Utils.GetAllTypes();
            foreach (var aType in all)
            {
                //if (aType.Name.Contains("BindableProperty"))
                //    Debug.Log("--->" + aType.Name);

                var index = files.FindIndex(s => { return s == aType.Name; });
                if (index >= 0)
                {
